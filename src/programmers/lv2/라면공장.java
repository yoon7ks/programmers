...
public class 라면공장 {
    ...
    public static void main(String[] args) {
        ...
        int k = 30;
        solution2(stock, dates, supplies, k);
    }
    ...
    public static int solution(int stock, int[] dates, int[] supplies, int k) {
        ...
        for (int i = 0; i < k; i++) {
            for (int j = lastDateIdx; j < dates.length; j++) {
                if (i == dates[j]) {
    ...
    public static int solution2(int stock, int[] dates, int[] supplies, int k) {
        ...
        // 우선순위: 들어온 순서에 상관없이 우선순위가 높은 데이터가 먼저 출력된다.
        PriorityQueue<Integer> queue = new PriorityQueue<>(Collections.reverseOrder());
        int lastDateIdx = 0;
        int factoryStock = stock;

        // 공장재고가 k보다 작은 경우
        while (factoryStock < k) {
            // 현재 인덱스가 공급일갯수보다 작고, 현재 공급일인덱스의 일자가 공장재고보다 작을때
            while (lastDateIdx < dates.length && dates[lastDateIdx] <= factoryStock) {
                    queue.add(supplies[lastDateIdx]); // 큐에 공급량을 넣는다.
                    // 공급받았으므로 인덱스를 +1한다.
                    lastDateIdx++;
            }

            factoryStock += queue.poll(); // 현재 큐에 있는 재고를 공급한다.
            answer++; // 공급받았으므로 ++1한다.
        }
