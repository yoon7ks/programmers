...
위와 같은 방식으로 합니다.
	...
	public static int solution(int n) {
		...
		if (n%2 == 0) {
			//짝수
			int mod = n/2;
			System.out.println("짝수="+mod);
			for (int i = 0; i <= mod; i++) {
				jump++;
				if (jump*2 == mod) {
					System.out.println("순간이동1!");
				}
			}
		} else {
			int line = n;
			...
			//홀수
			int mod = n/2;
			System.out.println("홀수="+mod);
			
			for (int i = 0; i <= mod; i++) {
				if (x*2 == mod) { // 현재 점프값 *2 가 기준값과 같으면 
					System.out.println("순간이동!");
					moving += jump*2;
					x = moving;
				} else if (x*2 % n-1 == 0) { // 점프값*2한 순간이동값 / 마지막 -1이 == 0인경우
					moving += jump*2;
					jump++;
					x = moving;
				} else {
			...
			System.out.println("jump="+jump + ", moving=" + moving);
		}
	...
	public static int solution1(int n) {
		...
		while (n != 0) {
			...
			// N/2가 ==0이면 짝수이므로 순간이동이 가능하다.
			if (n%2 == 0) {
				n = n/2;
			} else {
				// 홀수이면  N=N-1하고 건전지로 한칸 이동한다.
				n = n-1;
				ans++;
			}
		}
